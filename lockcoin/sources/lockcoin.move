...
public struct TAdminCap has key, store {
    id: UID
}

public struct StakePosititon has store{
    value: u64,
    timestamp: u64,
    expire: u64,
}
...
public struct LockEvent has drop, copy {
    sender: address,
    value: u64,
    timestamp: u64,
    expire: u64,
...
public struct UnlockEvent has drop, copy {
    sender: address,
    value: u64,
    timestamp: u64,
    emergency: bool
...
public entry fun setEmergency<TOKEN>(_admin: &TAdminCap, pool: &mut Pool<TOKEN<>>, emergency: u64, _ctx: &mut TxContext){
   // assert!(pool.emergency != emergency, ErrInvalidParams);
    if (emergency ==0) {
        pool.emergency = false;
    }else if (emergency ==1) {
        pool.emergency = true;
    }
}

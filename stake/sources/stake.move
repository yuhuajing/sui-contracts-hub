module stake::stake {
    ...
    use stake::stake_config;
    use sui::coin::{Coin};
    use sui::tx_context::{TxContext, sender};
    use stake::stake_config::GlobalConfig;
    use sui::coin;
   // use stake::math128;
    use sui::table;
    ...
    public struct StakePool<phantom S, phantom R> has key, store {
        id: UID,
        reward_per_sec: u64,
    ...
    public fun register_pool<S, R>(
       ...
       // timestamp_ms: u64,
        ...
        let current_time = clock::timestamp_ms(sclock) / 1000; //@todo review math div
    ...
    public fun stake<S, R>(
        ...
        let current_time = clock::timestamp_ms(sclock)/1000;
    ...
    fun update_accum_reward<S, R>(pool: &mut StakePool<S, R>, timestamp_now: u64) {
       // let current_time = get_time_for_last_update(pool, timestamp_now);
        let new_accum_rewards = accum_rewards_since_last_updated(pool, timestamp_now);

        pool.last_updated = timestamp_now;
    ...
    public struct StakeEvent has drop, store, copy {
        pool_id: address,
    ...
    public struct UnstakeEvent has drop, store, copy {
        pool_id: address,
        user_address: address,
        amount: u64,
        user_staked_amount: u64,
    ...
    public struct DepositRewardEvent has drop, store, copy {
        pool_id: address,
        user_address: address,
        amount: u64,
    ...
    public struct HarvestEvent has drop, store, copy {
        pool_id: address,
        user_address: address,
        amount: u64,
        staked_amount: u64,
    ...
    public struct RegisterPoolEvent has drop, copy, store {
        pool_id: address,
        reward_per_sec: u64,
        last_updated: u64,
        start_timestamp: u64,
